...
async def preview_results(
    ...
    file_type: output name (e.g., "a1", "a2", "a4", or any configured output)
    ...
    # Get file path based on file_type (supports dynamic output names)
...
async def download_file(
    ...
    if file_type not in ["a1", "a2", "report"]:
        # Accept any output name from file_results, but validate common ones
        pass
...
async def generate_report(
    ...
    try:
        # Parse file_results JSON (dynamic output paths)
...
async def get_batch_stats(
    ...
    # Dynamic output stats (from file_results) — unified for ALL outputs
    # Each output gets: total, by_{col} breakdowns, filter_columns
    output_stats = {}
        ...
        if output_path and os.path.exists(output_path):
            try:
                ...
                for col in df_out.columns:
                    col_lower = col.lower()
                    # Detect status-like columns (exclude detail/note — they're 1-1 with status)
                ...
                # A2 special: also break down by source
    ...
    if batch.config_id:
        ...
        if config:
            try:
                # V2: status_combine_rules is on WorkflowStep, not PartnerServiceConfig
    ...
    # Build output_order from workflow steps (step_order → output_name)
    # so frontend can sort outputs by step execution order, not alphabetically
    output_order = {}
    if batch.config_id:
        if not config:
            config = db.query(PartnerServiceConfig).filter(
