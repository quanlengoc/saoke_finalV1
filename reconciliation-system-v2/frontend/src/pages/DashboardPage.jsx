...
import { reconciliationApi, reconciliationApiV2, approvalsApi } from '../services/api'
import { useAuthStore } from '../stores/authStore'

export default function DashboardPage() {
  const { user } = useAuthStore()
  
  // Fetch recent batches - try V2 first, fallback to V1
  const { data: batchesData } = useQuery({
    queryKey: ['recentBatches'],
    queryFn: async () => {
      try {
        return await reconciliationApiV2.listBatches({ page: 1, page_size: 5 })
      } catch {
        return await reconciliationApi.listBatches({ limit: 5 })
      }
    },
  })
