...
import { reconciliationApi, reconciliationApiV2, partnersApi } from '../services/api'
...
export default function BatchListPage() {
  ...
  // Rerun mutation - try V2 first, fallback to V1
  const rerunMutation = useMutation({
    ...
    onError: (err) => {
      ...
      const errorDetail = Array.isArray(detail) 
        ...
        : (typeof detail === 'string' ? detail : err.message || 'Lỗi không xác định')
  ...
  // Fetch batches - try V2 first, fallback to V1
  const { data: batchesData, isLoading } = useQuery({
    queryKey: ['batches', filters],
    queryFn: async () => {
      try {
        return await reconciliationApiV2.listBatches(filters)
      } catch {
        return await reconciliationApi.listBatches(filters)
      }
    },
  })
