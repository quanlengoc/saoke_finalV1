...
class GenericMatchingEngine:
    ...
    def match_datasets(
        ...
        Returns:
            DataFrame with columns:
                - {left_name}_index: Index from left DataFrame
                - {right_name}_index: Index from right DataFrame (or None)
                - status: Match status
                - note: Description
                - amount_diff: Amount difference (if amount rule exists)
        """
        import time
    ...
    def _match_by_expression(
        ...
        left_unique = left_df['_match_key'].nunique()
        right_unique = right_df['_match_key'].nunique()
        ...
        right_deduped = right_df[['_right_idx', '_match_key']].drop_duplicates(
