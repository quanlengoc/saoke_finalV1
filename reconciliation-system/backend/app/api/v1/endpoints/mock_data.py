"""
Mock data management endpoints
...
def parse_mock_filename(filename: str) -> tuple:
    ...
    Expected format: {PARTNER_CODE}_{SERVICE_CODE}_b4_mock.csv
    """
...
async def upload_mock_file(
    ...
    The file will be saved as {PARTNER_CODE}_{SERVICE_CODE}_b4_mock.csv
    """
    # Validate file type
...
async def get_b4_columns(
    ...
    Useful for populating dropdowns in rule configuration UI
    """
    filename = f"{partner_code.upper()}_{service_code.upper()}_b4_mock.csv"
    mock_path = get_storage_path('mock_data') / filename
